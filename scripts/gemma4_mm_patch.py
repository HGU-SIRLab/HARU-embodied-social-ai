...
try:
    ...
    Gemma4UnifiedVisionConfig = None
...
class Gemma4ProcessingInfo(BaseProcessingInfo):
    def get_hf_config(self):
        try:
            ...
            # Also accept Gemma4UnifiedConfig (encoder-free vision variant)
            if Gemma4UnifiedVisionConfig is not None:
                try:
                    from transformers.models.gemma4_unified.configuration_gemma4_unified import (
                        Gemma4UnifiedConfig,
                    )
...
# ---------------------------------------------------------------------------
# Encoder-free vision embedder for Gemma4 Unified (gemma4_unified model type)
# ---------------------------------------------------------------------------


class Gemma4UnifiedVisionEmbedder(nn.Module):
    """Simple patch-based vision embedder for Gemma4 Unified.

    Replaces the SiGLIP vision_tower with a lightweight linear pipeline:
    raw patches → LN₁ → Linear → LN₂ → +factorized_posemb → LN₃ → RMSNorm → Linear

    Weight key mapping (checkpoint → vLLM module):
      model.vision_embedder.*           → vision_embedder.*
      model.embed_vision.embedding_projection.weight → vision_embedder.embedding_projection.weight
    """

    def __init__(self, config):
        super().__init__()
        vision_cfg = config.vision_config
        # model_patch_size = 48, so patch_dim = 48*48*3 = 6912
        patch_dim = getattr(vision_cfg, "model_patch_size", 48) ** 2 * 3
        mm_embed_dim = getattr(vision_cfg, "mm_embed_dim", config.text_config.hidden_size)
        posemb_size = getattr(vision_cfg, "mm_posemb_size", 1120)
        rms_eps = getattr(vision_cfg, "rms_norm_eps", 1e-6)
        ...
        self.pos_embedding = nn.Parameter(torch.zeros(posemb_size, 2, mm_embed_dim))
        self.pos_norm = nn.LayerNorm(mm_embed_dim)
        self.rms_eps = rms_eps
        # Final projection matching model.embed_vision.embedding_projection.weight
        self.embedding_projection = nn.Linear(mm_embed_dim, text_hidden, bias=False)
    ...
    def forward(
        ...
        """
        Args:
            pixel_values: (batch, num_patches, patch_dim) raw merged pixel patches.
            image_position_ids: (batch, num_patches, 2) integer XY positions (-1=padding).
        Returns:
            (batch, num_patches, text_hidden_size)
        """
        dtype = self.patch_dense.weight.dtype
        x = self.patch_ln1(pixel_values.to(dtype))
        x = self.patch_dense(x)
        x = self.patch_ln2(x)

        # Factorized 2-D positional embeddings
        clamped = image_position_ids.clamp(min=0).long()
        valid = (image_position_ids != -1).to(dtype).unsqueeze(-1)
        axes = torch.arange(2, device=image_position_ids.device)
        pos_embs = (self.pos_embedding[clamped, axes] * valid).sum(-2)
        x = x + pos_embs
        x = self.pos_norm(x)

        # RMSNorm without learnable scale, then project to LM space
        xf = x.float()
        x_normed = (xf * torch.rsqrt(xf.pow(2).mean(-1, keepdim=True) + self.rms_eps)).to(dtype)
        return self.embedding_projection(x_normed)
...
class Gemma4ForConditionalGeneration(
    ...
    # Maps checkpoint prefixes to vLLM module paths (standard Gemma4).
    ...
    # Mapper for Gemma4 Unified (gemma4_unified model_type).
    # Checkpoint uses model.vision_embedder.* and model.embed_vision.embedding_projection.*
    hf_to_vllm_mapper_unified = WeightsMapper(
        orig_to_new_prefix={
            "model.embed_audio.": "embed_audio.",
            "model.vision_embedder.": "vision_embedder.",
            "model.embed_vision.embedding_projection.": "vision_embedder.embedding_projection.",
            "model.language_model.": "language_model.model.",
            "lm_head.": "language_model.lm_head.",
            "model": "language_model.model",
        }
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Detect gemma4_unified: uses a simple linear embedder instead of SiGLIP.
        _vision_cfg_type = getattr(config.vision_config, "model_type", "")
        self._is_unified_vision = _vision_cfg_type == "gemma4_unified_vision"

        if self._is_unified_vision:
            # Encoder-free path: simple patch embedding (no SiGLIP tower).
            self.vision_embedder = Gemma4UnifiedVisionEmbedder(config)
        ...
        # gemma4_unified uses Gemma4UnifiedAudioConfig which is not AutoModel-registered.
        # Skip audio tower for unified models (HARU uses vision-only path).
        _audio_cfg_type = getattr(config.audio_config, "model_type", "")
        _audio_unified = _audio_cfg_type == "gemma4_unified_audio"

        if config.audio_config is not None and not _audio_unified:
    ...
    def _process_image_input(
        ...
        # gemma4_unified: encoder-free path (simple linear embedder, no SiGLIP)
        if self._is_unified_vision:
            per_image_features = []
            for i in range(pixel_values.shape[0]):
                pv = pixel_values[i].unsqueeze(0)   # (1, max_patches, 6912)
                pp = pixel_position_ids[i].unsqueeze(0)  # (1, max_patches, 2)
                # Forward returns (1, max_patches, text_hidden_size)
                feats = self.vision_embedder(pv, pp)
                # Strip padding patches (position_ids == -1 on both axes)
                padding_mask = (pp == -1).all(dim=-1).squeeze(0)  # (max_patches,)
                per_image_features.append(feats.squeeze(0)[~padding_mask])
            return per_image_features

        # Standard Gemma4 path: SiGLIP vision tower + multimodal embedder
    ...
    def _process_video_input(
        ...
        if self._is_unified_vision:
            # gemma4_unified: use vision_embedder for video frames too
            vt = None
            pooling_k2 = 1  # unused
            target_dtype = self.vision_embedder.embedding_projection.weight.dtype
        else:
            vt = self.vision_tower
            pooling_k2 = self.config.vision_config.pooling_kernel_size**2
            target_dtype = self.embed_vision.embedding_projection.weight.dtype
        ...
        for pv_chunk, pp_chunk in zip(pv_per_video, pp_per_video):
            ...
            for i in range(pv_chunk.shape[0]):
                ...
                if self._is_unified_vision:
                    # gemma4_unified: encoder-free embedder, strip padding
                    feats = self.vision_embedder(pv, pp)
                    padding_mask = (pp == -1).all(dim=-1).squeeze(0)
                    frame_emb = feats.squeeze(0)[~padding_mask]
                else:
                    max_patches = pv.shape[1]
                    output_length = max_patches // pooling_k2
                    vt_output = vt(pv, pp, output_length=output_length)
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        if self._is_unified_vision:
            # gemma4_unified: no vision_tower or embed_vision (standard) in model
            ignore_prefixes.extend(["vision_tower.", "embed_vision."])
            mapper = self.hf_to_vllm_mapper_unified
        else:
    ...
    def get_mm_mapping(self) -> MultiModelKeys:
        ...
        if self._is_unified_vision:
            connectors = ["vision_embedder"]
            tower_models: list[str] = []
        else:
